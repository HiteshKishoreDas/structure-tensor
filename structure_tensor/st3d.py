...
def structure_tensor_3d(volume, sigma, rho, out=None, truncate=4.0, mode='nearest'):
    ...
    Vx = filters.gaussian_filter(volume, sigma, order=[0, 0, 1], mode=mode, truncate=truncate)
    Vy = filters.gaussian_filter(volume, sigma, order=[0, 1, 0], mode=mode, truncate=truncate)
    Vz = filters.gaussian_filter(volume, sigma, order=[1, 0, 0], mode=mode, truncate=truncate)
    ...
    filters.gaussian_filter(tmp, rho, mode=mode, output=S[0], truncate=truncate)
    np.multiply(Vy, Vy, out=tmp)
    filters.gaussian_filter(tmp, rho, mode=mode, output=S[1], truncate=truncate)
    np.multiply(Vz, Vz, out=tmp)
    filters.gaussian_filter(tmp, rho, mode=mode, output=S[2], truncate=truncate)
    np.multiply(Vx, Vy, out=tmp)
    filters.gaussian_filter(tmp, rho, mode=mode, output=S[3], truncate=truncate)
    np.multiply(Vx, Vz, out=tmp)
    filters.gaussian_filter(tmp, rho, mode=mode, output=S[4], truncate=truncate)
    np.multiply(Vy, Vz, out=tmp)
    filters.gaussian_filter(tmp, rho, mode=mode, output=S[5], truncate=truncate)
